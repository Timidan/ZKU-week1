...
template sudoku() {
    ...
    component puzzleProof[9][9];
    component sProof[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puzzleProof[i][j]=RangeProof(32);
            sProof[i][j]=RangeProof(32);

            puzzleProof[i][j].in<==puzzle[i][j];
            puzzleProof[i][j].range[0]<==0;
            puzzleProof[i][j].range[1]<==9;

            sProof[i][j].in<==solution[i][j];
            sProof[i][j].range[0]<==0;
            sProof[i][j].range[1]<==9;

            assert(puzzleProof[i][j].out == 1);
            assert(sProof[i][j].out == 1);
